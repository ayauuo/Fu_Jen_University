...
/** bk02／bk03 拍照頁預覽框額外縮至 75%（在單張縮半等規則之後套用） */
const TEMPLATE_EXTRA_PREVIEW_SCALE: Partial<Record<string, number>> = {
  bk02: 0.75,
  bk03: 0.75,
}

/** 依版型套用拍照頁預覽框尺寸縮放（單張 halve → bk02/bk03 ×0.75） */
export function scaleShootPreviewSize(
  ...
  let hh = h
  if (shotCount <= 1) {
    ww = Math.max(1, Math.round(ww / 2))
    hh = Math.max(1, Math.round(hh / 2))
  }
  const extra = template?.id ? TEMPLATE_EXTRA_PREVIEW_SCALE[template.id] : undefined
  if (extra != null && extra > 0 && extra !== 1) {
    ww = Math.max(1, Math.round(ww * extra))
    hh = Math.max(1, Math.round(hh * extra))
  }
  const previewScale = template?.shootLayout?.previewWidthScale
  if (previewScale != null && previewScale > 0 && previewScale !== 1) {
    ww = Math.max(1, Math.round(ww * previewScale))
    hh = Math.max(1, Math.round(hh * previewScale))
  }
  return { w: ww, h: hh }
}
...
export function useTakePicture(selectedTemplate: () => Template | null) {
  ...
  async function setCoverAndVideoSize(index = 0): Promise<void> {
    ...
    const scaled = scaleShootPreviewSize(selectedTemplate(), shotCount.value, w, h)
    pictureAreaWidth.value = scaled.w
    pictureAreaHeight.value = scaled.h
