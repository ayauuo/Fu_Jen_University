...
if exist "%PUBLISH_DIR%\web\assets\templates\chooselayout\bk01.png" (
    if exist "%PUBLISH_DIR%\web\assets\templates\chooselayout\bk02.png" (
        if exist "%PUBLISH_DIR%\web\assets\templates\chooselayout\bk03.png" (
            echo ✓ 選版型圖片存在 ^(bk01/bk02/bk03^)
        ) else (
            echo ✗ 選版型圖片 bk03 不存在！
        )
    ) else (
